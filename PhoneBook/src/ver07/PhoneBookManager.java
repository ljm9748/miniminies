...
//		}
		...
		//	System.out.println(" 1. 기본"); // 추상 클래스로 -> 인스턴스 생성불가
			...
			try {
				...
				System.out.println("잘못된 메뉴를 입력했습니다. 다시 선택해주세요.");
				SC.nextLine();
				continue;
			}
			break;
...
//			cnt--; 		// 저장된 개수를 감소
	...
	// 20.10.27 기능 추가
	// phoneBook(List)에 저장되어 있는 인스턴스들을 저장
	// 
	public void save( ) {
		// 인스턴스를 저장할 수있는 출력 스트림 생성
		
		if(phoneBook.size()==0) {
			System.out.println("저장된 데이터가 없습니다. 파일 저장이 되지 않습니다.");
			return;
		}
...
//			}
	...
	// 프로그램으로 파이르이 저장 데이터를 로드 메소드
	void load() {
		// 파일의 존재 여부 확인 : File
		File file = new File("phonebook.ser"); // 프로젝트 안에서 파일을 찾는다.
		// 파일 존재 여부
		if(!file.exists()) {
		...
		// 파일에 있는 데이터를 메모리에 저장한다.(phonebook에 저장)
		// 예외 처리
		try {
