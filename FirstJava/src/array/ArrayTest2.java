...
public class ArrayTest2 {
	...
	public static void main(String[] args) {
		...
		// 2차원 배열의 참조: 배열이름[행의 index][열의 index] // index -> 0부터 시작
		arr[0][0] = 10;
		...
		System.out.println("arr.length: "+ arr.length); 		// 1차원 배열을 저장하는 요소 - 
																// 2차원 배열 arr의 length 값은 1차원 배열의 개수를 가진다.
																// arr의 length 값은 2차원 배열의 행의 개수다.
		System.out.println("arr[0].length: "+ arr[0].length);	// arr[0]는 첫번째 1차원 배열을 가리키는 주소값을 저장하는 변수이다.
		System.out.println("arr[1].length: "+ arr[1].length);	// arr[1]는 두번째 1차원 배열을 가리키는 주소값을 저장하는 변수이다.
		System.out.println("arr[2].length: "+ arr[2].length);	// arr[2]는 세번째 1차원 배열을 가리키는 주소값을 저장하는 변수이다.
		...
		// for - each 구문: 배열의 원본 데이터의 수정이 아닌 일괄 참조의 목적일 때 사용 가능
		int[] arr4 = {1, 2, 3, 4, 5};
		...
		// for-each 에서의 값의 변경 -> 배열에는 영향을 주지 않는다.
		for(int n : arr4) {
			n++;
			System.out.println(n);		
		}
	...
	// 정수형 2차원 배열을 전달받아 
	//모든 요소의 값을 출력하는 메소드 -> 반환하는 데이터는 없다.
